...
def inserir(nome, telefone, endereco, dataAtual):
    try:
        ...
        sql = "insert into pessoa(codigo, nome, endereco, telefone, dataAtual) values('','{}','{}','{}','{}')".format(nome, endereco, telefone, dataAtual)
...
def selecionar():
    try:
        ...
        for (codigo, nome, endereco, telefone, dataAtual) in con:
            print(codigo, nome, endereco, telefone, dataAtual)
        print('\n')
    except Exception as erro:
        print(erro)
...
def atualizarData(codigo, dataAtual):
    try:
        dataAtual = transformarData(dataAtual)
        sql = "update pessoa set data = '{}' where codigo = '{}'".format(dataAtual, codigo)
        con.execute(sql)
        db_connection.commit()
        print(con.rowcount, "Atualizado!")
    except Exception as erro:
